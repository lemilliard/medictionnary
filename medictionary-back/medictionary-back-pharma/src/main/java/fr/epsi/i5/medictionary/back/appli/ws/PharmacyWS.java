...
public class PharmacyWS {
    ...
    public Pharmacy getPharmacyWithDrugs(@RequestBody ParamPharma param) throws MDException {
        ...
        String rq = "select p.* from medictionary_pharma.pharmacy as p where p.id_pharmacy in(";
        for( String cas : param.casDrug) {
            if(isFirst){
                rq += "select s.id_pharmacy from medictionary_pharma.stock as s where (s.cas_drug = \'"+ cas +"\' and s.quantity > 0) ";
                isFirst = false;
            }else{
                rq += "and s.id_pharmacy in (select s.id_pharmacy from medictionary_pharma.stock as s where (s.cas_drug = \'"+ cas +"\' and s.quantity > 0)) ";
            }
        }
        rq += ")";
